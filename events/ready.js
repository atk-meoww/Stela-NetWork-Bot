...
module.exports = {
  ...
  async execute() {
    try {
      ...
      (async () => {
        try {
          if (!process.env.CLIENT_ID || !process.env.GUILD_ID || process.env.CLIENT_ID === process.env.GUILD_ID) {
            console.warn('[Ready] Bỏ qua đăng ký slash commands: CLIENT_ID hoặc GUILD_ID trong .env bị thiếu hoặc sai.');
            return;
          }
